...
export default function Projects() {
  return (
    <section id="Projects" className="py-[104px] max-md:py-16 w-[min(1184px,calc(100%-96px))] mx-auto max-md:w-[calc(100%-40px)]">
      ...
      <div className="grid gap-x-8 gap-y-12 md:grid-cols-2">
        {projectTexts.projects.map((project, index) => (
          <article key={project.name} className={`min-w-0 ${index === 0 ? 'col-span-full grid items-center gap-8 rounded-xl border border-border bg-card p-6 md:grid-cols-[1.35fr_1fr] lg:gap-12 lg:p-8 max-md:p-4 max-md:gap-4' : ''}`}>
            <a href={project.links.web} target="_blank" rel="noopener noreferrer" className={`block overflow-hidden rounded-lg border border-border pb-0 [&:hover_img]:scale-[1.025] ${previewBackgrounds[index % previewBackgrounds.length]} ${index === 0 ? "px-4 pt-4" : "px-5 pt-5"}`} aria-label={`Visit ${project.name} (opens in a new tab)`}>
